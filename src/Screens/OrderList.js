...
import { fetchAllOrdersRequest } from '../store/Slices/Order.Slice';
...
const OrderList  = () => {
	...
	const { list=[], pages = 1 }= useSelector(s => s.ordersListReducer) ?? {list:[], pages : 1};
	...
	useEffect(() => {
		dispatch(fetchAllOrdersRequest({ pageno: pageNo, perpage:perPage}))
	},[])
    useEffect(() => {
        window.scrollTo(0, 0);
    },[isOpen])
	...
	const tableHeader = [	
		"OrederID",
        'Order Item','Total Price','Shipping Charge','Payment Status','Delivery Status','Order Date','Delivery Date','','','',
        "Action"
    ]
	...
	return <div class="main-container">
            <Table header={ tableHeader } render = {tableUI} pages={pages} onClick={onClickPage} currentPage={pageNo} />
        <Modal 
            // isOpen={isOpen} 
